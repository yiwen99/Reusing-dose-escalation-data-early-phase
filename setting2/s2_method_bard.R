# =====================================================================
# Setting 2: BARD (backfill + adaptive randomization), class "bard"
# =====================================================================
# Arms (9): bard_{nomatch|clean|mis}_{car3|car2}, the matching axis crossed with
# the CAR axis, plus bfboin_sr_{nomatch|clean|mis}, the same matching axis with
# 1:1 simple randomization instead of covariate-adaptive randomization.
#
# bfboin_sr_* is not the published BOIN-SR comparator. It keeps BF-BOIN Stage 1
# with backfilling, the same retained Stage-1 records under the same matching
# rule, the same Stage-1-plus-Stage-2 combination at analysis, the same N2star,
# and the same OBD selectors. The only difference from BARD is the Stage-2
# assignment rule: an independent fair coin per patient instead of conditional
# Pocock-Simon minimization at gamma_car. The published BOIN-SR uses plain BOIN
# with no backfill and does not combine Stage-1 data, so it differs from BARD on
# three axes at once. Arm sizes are unequal by construction, so alloc_imb is
# larger than for the CAR arms.
#
# Note on precision: Setting 1 pre-draws one Stage-2 stream and shares it across
# arms, so its SR-versus-CAR contrast is paired. Setting 2 draws its Stage-2
# patients independently per arm (see run_stage2_car below), so the same contrast
# here carries Monte Carlo noise that Setting 1's does not, and the two settings'
# SR contrasts are not equally precise.
#
# Phantom MTD-1 dose: when Stage 1 returns mtd == 1 the candidate set becomes
# c(PHANTOM, 1), so there are always two candidates. The phantom is a decoy at
# 80 percent of dose 1's component-ii toxicity and efficacy with no Stage-1
# records under any matching mask; picking it is a selection failure, coded
# sel = -1, door = 3, reported as phantomSel, with its Stage-2 patients in
# phantom_n / phantom_pct.
#
# Retention (which Stage-1 patients qualify for combination):
#   nomatch : everyone (BARD as published; (1-lam) contamination, not
#             size-matched, so 70 percent at lam = 0.3)
#   clean   : C_M1 = C_M2 = 1 (true type-ii, oracle, no contamination)
#   mis     : C_M1 = C_M3 = 1 (proxy match; contamination =
#             (1-rho)*(1-sqrt(lam)) = 22.6 percent at lam = 0.3, rho = 0.5).
#             Size-matched to clean: both pools are exactly lam of Stage 1, since
#             C3 shares C2's Bern(sqrt(lam)) marginal.
# Matching is applied to the retained Stage-1 records before Stage 2; Stage 2 is
# pure component ii and never filtered. Arms differ in both retention and CAR, so
# per-arm Stage-2 is drawn independently.
#
# Requires setting2_config.R sourced first (S1/S2 tox/eff, beta0_i/beta0_ii,
# trueOBD, trueMTD, family, phiT, phiE, alpha, cutoff.eli, bP1..bP3, rho, lam,
# psi, calendar, NSIM, seed).
# =====================================================================
...
n2_single <- 20        # per-dose target when only one candidate exists. The
                       ...
                       # so the is.na(d_high) branch is unreachable as
                       # configured; it is kept so the file still behaves if the
                       # phantom is disabled.
gamma_car <- 0.95      # minimization assignment probability (Pocock-Simon)
# There is no Stage-2 cohort size: the BARD paper enrolls and assigns Stage 2 one
# patient at a time, with the conditional Pocock-Simon minimization recomputing
# the imbalance index from all Stage-1 and Stage-2 patients accrued so far, so a
# batch has no meaning unless the tallies are frozen within it. Cohorts appear in
# the paper only in Stage-1 escalation.
...
util_true <- function(pT, pE) 0.3 - 0.3 * pT + 0.7 * pE - 0.2 * pT * pE   # mean-utility curve

# ---------------------------------------------------------------------
# Gumbel joint binary draw and observed utility (BARD keeps its own definitions)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Single-patient draw from the mixture DGP (Stage 1) or pure ii (Stage 2)
# ---------------------------------------------------------------------
draw_pat_s1 <- function(s, d) {
  sql <- sqrt(lam)
  c1 <- rbinom(1,1,sql); c2 <- rbinom(1,1,sql)
  cp <- rbinom(1,1,rho); c3 <- if (cp == 1L) c2 else rbinom(1,1,sql)
  is_ii <- (c1 == 1L && c2 == 1L)
  cp1 <- rbinom(1,1,0.5); cp2 <- rbinom(1,1,0.5); cp3 <- rbinom(1,1,0.5)
  b0 <- if (is_ii) beta0_ii[s,d] else beta0_i[s,d]
  pE <- plogis(b0 + bP1*cp1 + bP2*cp2 + bP3*cp3)
  pT <- if (is_ii) S2_tox[s,d] else S1_tox[s,d]
  oc <- draw_gumbel(pT, pE, psi)
  dlt.time <- if (oc$tox == 1L) runif(1, 0, dlt.window) else dlt.window
  list(is_ii = is_ii, tox = oc$tox, eff = oc$eff, util = util_obs(oc$tox, oc$eff),
       cp1 = cp1, cp2 = cp2, cp3 = cp3,
       ret_clean = (c1 == 1L && c2 == 1L), ret_mis = (c1 == 1L && c3 == 1L),
       dlt.time = dlt.time)
}
draw_cov_s2 <- function() {
...
# be PHANTOM. draw_pat_s1 above is Stage-1 only, where the phantom cannot occur,
# so it keeps its direct beta0_i / beta0_ii lookups.
outcome_s2 <- function(s, d, cov) {
  pE <- plogis(b0_at(s, d) + bP1*cov$cp1 + bP2*cov$cp2 + bP3*cov$cp3)
  oc <- draw_gumbel(tox_at(s, d), pE, psi)
...
# ---------------------------------------------------------------------
# Stage 1: BF-BOIN with backfill on the mixture DGP
# ---------------------------------------------------------------------
run_stage1_bard <- function(s) {
  maxpat <- ncohort * cohort1 + J * n_cap + cohort1   # generous preallocation
  rec_dose <- integer(maxpat); rec_ii <- logical(maxpat); rec_eff <- integer(maxpat)
  rec_tox <- integer(maxpat); rec_util <- numeric(maxpat)
  rec_cp1 <- integer(maxpat); rec_cp2 <- integer(maxpat); rec_cp3 <- integer(maxpat)
  rec_rc <- logical(maxpat); rec_rm <- logical(maxpat)
  nrec <- 0L
  add_rec <- function(d, p) {
    nrec <<- nrec + 1L
    rec_dose[nrec] <<- d; rec_ii[nrec] <<- p$is_ii; rec_eff[nrec] <<- p$eff
    rec_tox[nrec] <<- p$tox; rec_util[nrec] <<- p$util
    rec_cp1[nrec] <<- p$cp1; rec_cp2[nrec] <<- p$cp2; rec_cp3[nrec] <<- p$cp3
    rec_rc[nrec] <<- p$ret_clean; rec_rm[nrec] <<- p$ret_mis
  }

  n_esc <- integer(J); y_esc <- integer(J)                    # escalation only
  bf_cal <- matrix(NA_real_, nrow = n_cap + 1L, ncol = J)     # backfill DLT maturation calendar
  ...
  cal.cohort <- rep(NA_real_, cohort1)   # DLT maturation calendar for current cohort
  evt.cohort <- integer(cohort1)         # DLT events for current cohort
  ...
  mtd <- if (any(elim == 1L) && elim[1] == 1L) NA_integer_ else
  ...
  recs <- data.frame(
    dose = rec_dose[seq_len(nrec)], is_ii = rec_ii[seq_len(nrec)],
    eff = rec_eff[seq_len(nrec)], tox = rec_tox[seq_len(nrec)], util = rec_util[seq_len(nrec)],
    cp1 = rec_cp1[seq_len(nrec)], cp2 = rec_cp2[seq_len(nrec)], cp3 = rec_cp3[seq_len(nrec)],
    ret_clean = rec_rc[seq_len(nrec)], ret_mis = rec_rm[seq_len(nrec)],
...
# ---------------------------------------------------------------------
# Stage 2: covariate-adaptive randomization, combine, select
# ---------------------------------------------------------------------
# ret : Stage-1 records at the two candidate doses that pass the arm's matching
#       rule (already filtered by the caller).
...
# randomization (independent fair coin per patient), the BF-BOIN-SR arms.
run_stage2_car <- function(s, cand, ret, car_idx, engine = "car") {
  ...
  cix_of <- function(tox, eff) 1L + eff + 2L * tox
  ...
  # The phantom contributes zero to n_an (it has no Stage-1 records under any
  # mask), so N2star is automatically larger in the mtd == 1 case.
  N2star <- max(0L, N2 - sum(n_an))
  n1_seed <- n_an
  ...
  if (engine == "car" && CAR_ENGINE == "minirand" && !is.na(d_high) && N2star > 0L) {
    covmat_all <- rbind(as.matrix(ret[, c("cp1", "cp2", "cp3")]),
                        matrix(NA_integer_, N2star, 3,
                               dimnames = list(NULL, c("cp1", "cp2", "cp3"))))
    res_vec <- c(match(ret$dose, cand), rep(100L, N2star))
  }

  # Sequential, one patient at a time: the assignment of each newly enrolled
  # Stage-2 patient minimizes the imbalance index computed from all Stage-1 and
  # Stage-2 patients accrued so far.
  if (!is.na(d_high) && N2star > 0L) {
    for (q in seq_len(N2star)) {
      cov <- draw_cov_s2()                       # ONE covariate draw per patient
      x1 <- cov$cp1; x2 <- cov$cp2; x3 <- cov$cp3
      if (engine == "sr") {
        # Independent fair coin, no covariate or allocation balancing. Setting 2
        # has no shared Stage-2 stream, so the coin is drawn here rather than read
        # off a pre-drawn stream.
        target_ci <- if (runif(1) < 0.5) 1L else 2L
      } else if (CAR_ENGINE == "minirand") {
        j <- n1_rows + q
        covmat_all[j, ] <- c(x1, x2, x3)
        if (j == 1L) {
          target_ci <- sample.int(2, 1)          # nothing accrued yet; ratio 1:1
        } else {
          target_ci <- Minirand(covmat = covmat_all[1:j, car_idx, drop = FALSE], j = j,
        ...
        if (D[1] == D[2]) target_ci <- sample.int(2, 1) else {
          amin <- which.min(D); target_ci <- if (runif(1) < gamma_car) amin else (3L - amin)
        }
      }
      po <- outcome_s2(s, cand[target_ci], cov)  # outcome at the ASSIGNED dose, same covariates
  ...
  } else if (is.na(d_high)) {
    # Unreachable as configured: with the phantom, length(cand) is always 2
    # whenever mtd is not NA, so d_high is never NA. Kept so the file still
    # behaves if the phantom is disabled.
    N2star <- max(0L, n2_single - n_an[1])
    if (N2star > 0L) {
      for (q in seq_len(N2star)) {
        cov <- draw_cov_s2()
        po  <- outcome_s2(s, cand[1], cov)
        n_an[1]  <- n_an[1]  + 1L
  ...
  s2_dur <- if (s2_dur_pats > 0)
    (if (accrual == "poisson") s2_dur_pats / n.per.month else s2_dur_pats / n.per.month) + eff.window
  else 0
...
# ---------------------------------------------------------------------
# Arm grid: the three matching rules crossed with the two CAR engines, then the
# same three matching rules with simple randomization. The order matters, because
# the per-arm result matrices are positional and the plotting registries key off
# these names.
ARMS <- data.frame(
  match  = c(rep(c("nomatch","clean","mis"), 2), "nomatch", "clean", "mis"),
  engine = c(rep(c("car3","car2"), each = 3), rep("sr", 3)),
  stringsAsFactors = FALSE)
ARMS$name <- ifelse(ARMS$engine == "sr",
                    sprintf("bfboin_sr_%s", ARMS$match),
                    sprintf("bard_%s_%s", ARMS$match, ARMS$engine))
# `car` is an output column of the table schema; it is NA on the SR arms, which
# balance no covariates.
ARMS$car <- ifelse(ARMS$engine == "sr", NA_character_, ARMS$engine)
NARM <- nrow(ARMS)
# car_of is called with the engine name; it returns integer(0) for "sr", where
# car_idx is unused because the SR branch balances nothing.
car_of <- function(engine) if (engine == "sr") integer(0) else
                           if (engine == "car3") 1:3 else 1:2
retain_mask <- function(recs, match) {
  if (match == "nomatch") rep(TRUE, nrow(recs))
  else if (match == "clean") recs$ret_clean
  else recs$ret_mis
}
...
one_rep <- function(s) {
  ...
  mtd <- st1$mtd
  expo1 <- as.integer(table(factor(st1$recs$dose, levels = 1:J)))
  ...
  for (a in 1:NARM) {
    mask <- retain_mask(st1$recs, ARMS$match[a])
    # The phantom has no records under any mask, so filtering cand to real doses
    # in the %in% test is all that is needed: `ret` contains no rows for it.
    ret  <- st1$recs[mask & st1$recs$dose %in% cand[cand != PHANTOM], , drop = FALSE]
    st2  <- run_stage2_car(s, cand, ret, car_of(ARMS$engine[a]),
                           engine = if (ARMS$engine[a] == "sr") "sr" else "car")
...
run_bard <- function(cfg) {
  ...
  for (s in 1:15) {
    ...
    for (a in 1:NARM) {
      ...
      if (any(selected)) { Est <- mean(ea[selected], na.rm = TRUE); True <- mean(S2_eff[s, sa[selected]]) }
      else { Est <- NA; True <- NA }
      ...
      full_rows[[si]] <- data.frame(
        ...
        method = ARMS$name[a], match = ARMS$match[a],
        engine = ARMS$engine[a], car = ARMS$car[a],
        trueOBD = ob, trueMTD = mt,
  ...
  od <- cfg$outdir
  write.csv(full, file.path(od, "setting2_bard_results_full.csv"),        row.names = FALSE)
  write.csv(sel_tab,     file.path(od, "setting2_bard_selection.csv"),  row.names = FALSE)
  write.csv(dist_tab,    file.path(od, "setting2_bard_seldist.csv"),    row.names = FALSE)
  write.csv(alloc_tab,   file.path(od, "setting2_bard_allocation.csv"), row.names = FALSE)
  write.csv(metrics_tab, file.path(od, "setting2_bard_metrics.csv"),   row.names = FALSE)

  cat("[bard] wrote setting2_bard_{selection,seldist,allocation,metrics}.csv and results_full to", od, "\n")
